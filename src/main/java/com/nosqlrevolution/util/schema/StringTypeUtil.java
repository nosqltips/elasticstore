...
public class StringTypeUtil {
    public static String generateSchema(StringType anno, String name) {
        try {
            XContentBuilder builder = JsonXContent.contentBuilder();
            builder.startObject(name).field(Field.TYPE.getName(), Type.STRING.getName());
                ...
                if (anno.term_vector() != VECTOR.DEFAULT) {
                    builder.field(Field.TERM_VECTOR.getName(), anno.term_vector().name());
                }
                if (anno.boost() != 1.0f) {
                ...
                if (anno.omit_norms() != OMIT_NORMS.DEFAULT) {
                    builder.field(Field.OMIT_NORMS.getName(), anno.omit_norms().name());
                }
                if (anno.omit_term_freq_and_positions() != OMIT_TERM_FREQ.DEFAULT) {
                    builder.field(Field.OMIT_TERMS.getName(), anno.omit_term_freq_and_positions().name());
                }
